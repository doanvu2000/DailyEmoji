package com.fansipan.habit.tracker.mood.data.database
...
import com.fansipan.habit.tracker.mood.screen.report.IconRank
import com.fansipan.habit.tracker.mood.screen.report.IconRankEntity
import com.fansipan.habit.tracker.mood.utils.CalendarUtil
import com.fansipan.habit.tracker.mood.utils.Constant
import com.fansipan.habit.tracker.mood.utils.isSdkQ
...
class BeanViewModel(private val repository: BeanRepository) : ViewModel() {
    ...
    val allBeanImageAttach: LiveData<List<BeanImageAttachEntity>> = repository.allBeanImageAttachEntity.asLiveData()
    ...
    fun insertBean(beanDailyEntity: BeanDailyEntity, onSuccess: ((beanId: Long) -> Unit)? = null) = viewModelScope.launch {
    ...
    fun getAllBeanIconById(id: Int, onSuccess: ((List<BeanIconEntity>) -> Unit)? = null) = viewModelScope.launch {
        onSuccess?.invoke(repository.getAllBeanIconById(id))
    }

    fun getAllBeanIcon(onSuccess: ((List<BeanIconEntity>) -> Unit)? = null) = viewModelScope.launch {
    ...
    //bean attach image
    fun getAllBeanImageAttachById(beanId: Int, onSuccess: ((List<BeanImageAttachEntity>) -> Unit)? = null) = viewModelScope
        .launch {
            onSuccess?.invoke(repository.getAllBeanImageAttach(beanId))
        }

    fun getAllBeanImageAttach(onSuccess: ((List<BeanImageAttachEntity>) -> Unit)? = null) = viewModelScope.launch {
        onSuccess?.invoke(repository.getAllBeanImageAttach())
    }

    fun insertBeanImageAttach(beanImageAttachEntity: BeanImageAttachEntity) = viewModelScope.launch {
    ...
    //music calm
    fun getAllMusicCalm(onSuccess: ((List<MusicCalmEntity>) -> Unit)? = null) = viewModelScope.launch {
        onSuccess?.invoke(repository.getAllMusicCalm())
    }

    fun getSecondMusicCalmWithMonth(month: Int, year: Int, onSuccess: ((second: Int) -> Unit)) = viewModelScope.launch {
...
//        val APP_MEDIA_FOLDER = "doan"
    ...
    private val listType = listOf(8, 7, 3, 4, 6, 5, 2, 1)
    private val color1 = Color.rgb(53, 201, 114)
    private val color2 = Color.rgb(255, 128, 75)
    private val color3 = Color.rgb(131, 138, 184)
    private val color4 = Color.rgb(254, 121, 166)
    private val color5 = Color.rgb(255, 207, 74)
    private val color6 = Color.rgb(180, 231, 130)
    private val color7 = Color.rgb(90, 168, 248)
    private val color8 = Color.rgb(208, 230, 247)
    private val listColorType = listOf(
        color8, color7, color3, color4, color6, color5, color2, color1
    )
    ...
    fun getDataListIconRanking(
        ...
        val beansPrevious = listBeanDailyEntity.filter { it.month == newMonth && it.year == newYear }
    ...
    fun getAvgBedTime(beans: List<BeanDailyEntity>): Int {
        return CalendarUtil.getHourAverage(beans.filter { it.timeGoToBed != null }.map { it.timeGoToBed ?: "00:00" })
    }

    fun getAvgWakeTime(beans: List<BeanDailyEntity>): Int {
        return CalendarUtil.getHourAverage(beans.filter { it.timeWakeup != null }.map { it.timeWakeup ?: "00:00" })
    }
