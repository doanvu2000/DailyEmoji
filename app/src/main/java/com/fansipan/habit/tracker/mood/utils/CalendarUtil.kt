...
object CalendarUtil {
    ...
    enum class DayOfWeek(val order: Int, val stringId: Int, val stringShort: Int) {
        ...
        companion object {
            ...
            fun getWeekTitleShort() = values().map { it.stringShort }
        }
    }
    ...
    fun isFuture(context: Context, month: String) =
    ...
    fun getHourAverage(list: List<String>): Int {
        if (list.isEmpty()) {
            return 0
        }
        val totalMinutes = list.sumOf {
            val minutes = convertStringToHour(it)
            if (minutes < 12 * 60) {
                minutes + 720
            } else {
                minutes - 720
        ...
        val avg = totalMinutes / (list.size * 1.0f)
        return if (avg > 12 * 60) {
            (avg - 12 * 60 + 0.5).toInt()
        } else {
            (avg + 12 * 60 + 0.5).toInt()
    ...
    //String format: 07:11
    fun convertStringToHour(string: String): Int {
        if (string == "") return 0
        val split = string.split(":")
        val hour = split[0].toIntOrNull() ?: 0
        val minutes = split[1].toIntOrNull() ?: 0
